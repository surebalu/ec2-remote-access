...
- **Terminal colour schemes and font**: Settings → Terminal offers Dracula, One Dark, Tokyo Night, Catppuccin Mocha, Nord, Gruvbox, Monokai, Solarized (dark/light), GitHub (dark/light) and One Light, plus any installed monospace font (Monaco by default) and size. Changes apply to open tabs immediately. A colour scheme only sets the palette; servers with a plain prompt still print plain text, so **Settings → Run after connect** can send a line to every new SSH shell (a one-click preset sets a green/blue `PS1` and `ls --color`), overridable per host in the connect dialog.
